...
def search_candidates(raw_query: str, top_k: int = 20) -> CandidateSearchResult:
    """
    Выполняет полный сценарий поиска кандидатов и возвращает структурированный результат.
    """
    log_event(
        "search_raw_query",
        {"raw_query": raw_query},
    ...
    if not getattr(norm, "is_hr_relevant", True) or not getattr(
        ...
        return CandidateSearchResult(
            raw_query=raw_query,
            normalized_query=getattr(norm, "normalized_query", ""),
            is_hr_relevant=getattr(norm, "is_hr_relevant", False),
            should_search_candidates=getattr(norm, "should_search_candidates", False),
            short_explanation=getattr(norm, "short_explanation", ""),
            request_type=getattr(norm, "request_type", "unclear"),
            intent=getattr(norm, "intent", "nonsense"),
            confidence=getattr(norm, "confidence", 0.0),
            candidates=[],
        )

    normalized_query = getattr(norm, "normalized_query", raw_query)
    matches = search_vectors(normalized_query, top_k=top_k)
    ...
    return CandidateSearchResult(
        ...
        normalized_query=normalized_query,
        is_hr_relevant=getattr(norm, "is_hr_relevant", True),
        should_search_candidates=getattr(norm, "should_search_candidates", True),
        short_explanation=getattr(norm, "short_explanation", ""),
        request_type=getattr(norm, "request_type", "unclear"),
        intent=getattr(norm, "intent", "candidate_search"),
        confidence=getattr(norm, "confidence", 1.0),
